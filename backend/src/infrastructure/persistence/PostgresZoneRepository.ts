...
export class PostgresZoneRepository implements IZoneRepository {
  ...
  async save(zone: Zone): Promise<void> {
    const data = {
      ...
      createdBy: zone.getCircleId().getValue(), // Using circleId as createdBy for now
      name: zone.getName(),
      nameEn: zone.getNameEn() ?? null,
      colorHex: zone.getColorHex().getValue(),
      polygonVertices: zone.getPolygon().getVertices() as any,
  ...
  private toDomain(record: any): Zone {
    const vertices = record.polygonVertices as Vertex[];
    return Zone.create({
