...
export default function LoginPage() {
  ...
  const [error, setError] = useState(
    searchParams.get('error') ? getErrorMessage(searchParams.get('error')!) : '',
  );
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (status === 401)
        setError('Email o contraseña incorrectos. Si no tienes cuenta, puedes crear una.');
      else if (status === 404)
        setError('No existe una cuenta con este correo. Crea una cuenta para empezar.');
      else if (status === 429)
  ...
  return (
    <div className="min-h-screen flex">
      ...
      <div className="hidden lg:flex lg:w-1/2 relative overflow-hidden">
        ...
        <BrandMark variant="dark" size="md" className="absolute left-8 top-8 z-20" />
        <div className="relative z-10 flex flex-col items-center justify-center w-full px-12">
          ...
          >
            ...
            <p className="text-xl text-white/70 max-w-md">
              Comparte tu ubicación con las personas que más importan. Seguro, privado y en tiempo
              real.
            </p>
          </motion.div>
          {/* Floating elements */}
      ...
      <div className="flex-1 flex items-center justify-center bg-background px-6 py-12 lg:px-12">
        ...
        >
          ...
          <div className="mb-8 lg:mb-10">
            <BrandMark size="lg" showText={false} className="justify-center mb-4 lg:hidden" />
            <h1 className="text-center text-3xl font-bold text-text-primary lg:hidden">
              FamilyLink
            </h1>
            <h2 className="text-2xl font-semibold text-text-primary hidden lg:block">
              Bienvenido de vuelta
            </h2>
            <p className="mt-2 text-center text-text-secondary text-sm lg:text-left">
          ...
          <form onSubmit={handleSubmit} className="space-y-5" noValidate>
            ...
            {error && (
              <motion.div
                ...
                animate={{ opacity: 1, y: 0 }}
                className="bg-error/10 border border-error/20 rounded-[12px] px-4 py-3"
              >
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full py-3.5 bg-accent hover:bg-accent-hover text-white font-semibold rounded-[14px] transition-colors disabled:opacity-50"
            >
